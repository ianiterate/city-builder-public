...
victory. Difficulty halves or doubles them. After a raid there are 70 to 140 quiet days.
...
- **Hold the walls**: the militia fights only raiders within 8 tiles of their barracks.
- **Go out and fight**: the militia chases the nearest raider anywhere.

Everyone else hides indoors on their own, eating from the house larder, and comes out when no
raider is within 6 tiles. The banner can be folded to a chip and unfolds when the raid changes
phase: warning, marching, at the walls, fighting, looting, leaving.
...
- A palisade piece cannot be placed on a finished building's last open side, so you cannot wall
  someone into their house by accident. Gates are always allowed; people walk through them.
- If a closed ring with no gate leaves anyone without a path to a storehouse holding food, the
  steward raises "walled in", naming how many are shut in, and clears it when the path is open.
- A freshly drawn run shows at once as a pale, low ghost and rises to full height as it is built.

## How a raid ends

A raid is over when every raider is dead or after about two days when the survivors ride home
with what they carry. Each raider fills a sack with up to 8 of the most valuable goods from the
fattest store. A raider killed while carrying drops the load back into the nearest store.

The raid counts as **repelled** if nothing was carried off, or at least half the band was killed.
A sacking costs every villager in the settlement 0.15 happiness. Either way the house cools by 0.1
